...
import edu.nwmissouri.universitypolice.saferidehome.service.SafeRideHomeReportService;
import edu.nwmissouri.universitypolice.saferidehome.service.SafeRideHomeService;


@SuppressWarnings("serial")
@WebServlet(name = "SafeRideHomeController", urlPatterns = { "/safeRideHomeController" })
public class SafeRideHomeController extends HttpServlet {
	...
	protected void processRequest(HttpServletRequest request,
		...
		try {
			...
			SafeRideHomeReportService reportUtil;
			String userId;
			String fromAddress;
			String toAddress;
			...
			if (requestFrom.equalsIgnoreCase("login")) {
				validatedUser = util.validateUser(
						request.getParameter("username"),
						request.getParameter("password"));
				if (validatedUser == null) {
					forwardRequest(
							"/index.jsp?msg=Invalid User ID or Password. Please try again.",
							request, response);
				}
				if (validatedUser.getAccountStatus() != null
						&& validatedUser.getAccountStatus().equalsIgnoreCase(
								"REGISTERED")) {
					forwardRequest(
							"/index.jsp?msg=Please validate your account before you can login to the system.",
							request, response);
				}
				if (validatedUser.getAccountStatus() != null
						&& validatedUser.getAccountStatus().equalsIgnoreCase(
								"BLOCKED")) {
					forwardRequest(
							"/index.jsp?msg=Your account has been blocked. Please contact the admin.",
							request, response);
				} else if (validatedUser.getUserType()
				...
				} else if (validatedUser.getUserType().equalsIgnoreCase(
						"STUDENT")) {
					session.setAttribute("user", validatedUser);
					forwardRequest(
							"/studentHome.jsp?name="
									+ request.getParameter("username"),
							request, response);
				} else if (validatedUser.getUserType().equalsIgnoreCase(
						"DISPATCHER")) {
					...
					try {
						driverVehicleMap = (HashMap<String, String>) getServletContext()
								.getAttribute("driverVehicleMap");
						if (driverVehicleMap == null) {
							driverVehicleMap = new HashMap(1, 1);
							applicationContext.setAttribute("driverVehicleMap",
									driverVehicleMap);
						}
					...
					forwardRequest("/driverSelectVehicle.jsp", request,
							response);
				} else {
					forwardRequest(
							"/index.jsp?msg=Oops! Something went wrong. Please try again later.",
							request, response);
				}
				...
				applicationContext = getServletContext();
				driverVehicleMap = (HashMap) applicationContext
						.getAttribute("driverVehicleMap");
				if (driverVehicleMap == null) {
					driverVehicleMap = new HashMap(1, 1);
					applicationContext.setAttribute("driverVehicleMap",
				...
				if (driverVehicleMap.containsValue(vehicleId)) {
					...
					try {
						driverVehicleMap = (HashMap<String, String>) getServletContext()
								.getAttribute("driverVehicleMap");
						driverVehicleMap.remove(((User) session
					...
					request.setAttribute("vehicleList", vehicles);
					forwardRequest(
							"/driverSelectVehicle.jsp?msg=The vehicle has already been taken by other driver",
							request, response);
				} else {
				...
				try {
					request.setAttribute(
							...
							(new ArrayList<String>(
									((HashMap<String, String>) getServletContext()
											.getAttribute("driverVehicleMap"))
											.keySet())));
				} catch (Exception e) {
				}
				...
				try {
					...
					user = new User(userId, request.getParameter("password"),
							"STUDENT", request.getParameter("name"),
							request.getParameter("email"),
							request.getParameter("phone"), "Registered");
					...
					if (util.addUser(user)) {
						if (action != null
								&& action.equalsIgnoreCase("addUser")) {
							forwardRequest(
									"/adminAddUser.jsp?msg=User added successfully",
									request, response);
						} else {
							forwardRequest("/signupSuccess.jsp", request,
									response);
						}
						...
						if (action != null
								&& action.equalsIgnoreCase("addUser")) {
							forwardRequest(
									"/adminAddUser.jsp?msg=Oops! Something went wrong. Please try again later",
									request, response);
						} else {
							forwardRequest(
									"/index.jsp?msg=Oops! Something went wrong. Please try again later.",
				...
				try {
					user = (User) session.getAttribute("user");
					User editedUser;
					editedUser = new User(user.getUserId(),
							request.getParameter("password"),
							user.getUserType(), request.getParameter("name"),
							request.getParameter("email"),
							request.getParameter("phone"), "Registered");
					...
					if (util.updateUser(editedUser)) {
						if (user.getUserType().equalsIgnoreCase("STUDENT")) {
							forwardRequest(
									"/studentHome.jsp?msg=Your changes have been saved.",
									request, response);
						} else if (user.getUserType().equalsIgnoreCase(
								"DISPATCHER")) {
							forwardRequest(
									"/dispatcherHome.jsp?msg=Your changes have been saved.",
									request, response);
						} else if (user.getUserType()
								.equalsIgnoreCase("DRIVER")) {
							forwardRequest(
									"/driverHome.jsp?msg=Your changes have been saved.",
									request, response);
						} else if (user.getUserType().equalsIgnoreCase("ADMIN")) {
							forwardRequest(
									"/adminHome.jsp?msg=Your changes have been saved.",
									request, response);
						}
					} else {
						forwardRequest(
				...
				try {
					if (request.getParameter("userId") != null) {
						util.updateUserStatus(request.getParameter("userId"),
				...
				try {
					try {
						capacity = Integer.parseInt(request
					...
					Vehicle vehicle = new Vehicle(
							...
							request.getParameter("status"), capacity,
				...
				forwardRequest("/adminAddVehicle.jsp?msg=" + msg, request,
						response);
			} else if (requestFrom.equalsIgnoreCase("requestRide")) {
				try {
					if (getServletContext().getAttribute("serviceStatus") == null) {
						forwardRequest(
								"/studentHome.jsp?msg=The service is not available right now. The hours of operations are Wed:4PM-2AM, Thu:10PM-2AM, Fri:4PM-4AM, Sat:10PM-2AM",
								request, response);
					} else {
						user = (User) session.getAttribute("user");
						String rideId = util.getRideId(user.getUserId());
						if (rideId.equalsIgnoreCase("")) {
							forwardRequest("/studentRequestRide.jsp", request,
									response);
						} else {
							forwardRequest(
									"/studentHome.jsp?msg=Your ride request, "
											+ rideId
											+ " is still to be/being served. You cannot request another one.",
									request, response);
						}
					}
				} catch (Exception e) {
				...
				try {
					...
					noOfRiders = Integer.parseInt(request
							.getParameter("noOfRiders"));
					if (user.getUserType() != null
							&& user.getUserType().equalsIgnoreCase("STUDENT")) {
						requestedBy = user;
					} else {
						requestedBy = new User();
						requestedBy.setUserId(request
								.getParameter("student919"));
					...
					ride.setCallbackNumber(request
							.getParameter("callBackNumber"));
					if (user.getUserType() != null
							&& user.getUserType().equalsIgnoreCase("DRIVER")) {
						rides = util.addRideAndGetRidesListForDriver(ride);
						if (rides == null) {
							forwardRequest(
									"/scheduleRide.jsp?msg=Oops! Something went wrong. Please try again later",
									request, response);
						}
						request.setAttribute("ridesList", rides);
						forwardRequest("/driverRideRequests.jsp", request,
								response);
					} else {
						...
						if (!"".equals(rideId)) {
							if (user.getUserType().equalsIgnoreCase("STUDENT")) {
								forwardRequest(
										"/studentRideRequestSuccess.jsp?rideId="
												+ rideId, request, response);
							} else {
								forwardRequest(
										"/scheduleRideSuccess.jsp?rideId="
												+ rideId, request, response);
							}
						}
					}
				...
				if (user.getUserType().equalsIgnoreCase("STUDENT")) {
					forwardRequest(
							"/studentHome.jsp?msg=Your ride request has been cancelled",
							request, response);
				} else if (user.getUserType().equalsIgnoreCase("DISPATCHER")) {
					forwardRequest(
							"/dispatcherHome.jsp?msg=Ride Request has been cancelled",
							request, response);
				}
				...
				try {
					...
					try {
						ride.setNoOfRiders(Integer.parseInt(request
								.getParameter("totalRiders" + rowNumber)));
					} catch (Exception e) {
						System.out.println(e);
					}
					try {
						ride.setNoOfFemaleRiders(Integer.parseInt(request
								.getParameter("femaleRiders" + rowNumber)));
					} catch (Exception e) {
						ride.setNoOfFemaleRiders(0);
						System.out.println(e);
					}
					try {
						ride.setNoOfMaleRiders(Integer.parseInt(request
								.getParameter("maleRiders" + rowNumber)));
					} catch (Exception e) {
						ride.setNoOfMaleRiders(0);
						System.out.println(e);
					}
					ride.setCoRiders(request.getParameter("coRiders"
							+ rowNumber));
					ride.setDriverComments(request.getParameter("comments"
							+ rowNumber));
					...
					if (driver != null
							&& !"null".equalsIgnoreCase(driver.trim())
							&& !"".equalsIgnoreCase(driver.trim())) {
						ride.setDriver(driver);
					}
					ride.setUser(user.getUserId());
					try {
						ride.setVehicle(((HashMap<String, String>) getServletContext()
								.getAttribute("driverVehicleMap")).get(driver));
					} catch (Exception e) {
						System.out.println(e);
					}
					try {
					...
					if (user.getUserType().equalsIgnoreCase("DRIVER")) {
						rides = util.getRidesListForDriver(user.getUserId());
						request.setAttribute("ridesList", rides);
						forwardRequest("/driverRideRequests.jsp?msg="
						...
						forwardRequest("/dispatcherRideRequests.jsp?msg="
								+ errorMessage, request, response);
					}
				...
				try {
					userId = request.getParameter("userId");
					try {
						User verifiedUser = util.updateUserStatus(userId,
								"ACTIVE");
						session.setAttribute("user", verifiedUser);
						if (verifiedUser != null
								&& verifiedUser.getUserType() != null) {
							if (verifiedUser.getUserType().equalsIgnoreCase(
									"ADMIN")) {
								forwardRequest(
										"/adminHome.jsp?name="
										...
										request, response);
							} else if (verifiedUser.getUserType()
									.equalsIgnoreCase("DISPATCHER")) {
								forwardRequest(
										"/dispatcherHome.jsp?name="
												+ verifiedUser.getName()
												+ "&msg=Your account has been verified!",
										request, response);
							} else if (verifiedUser.getUserType()
					...
					.equalsIgnoreCase("getStudentPhoneNumberAndName")) {
				try {
					String student919 = request.getParameter("student919");
					try {
						out.println(util
								.getStudentPhoneNumberAndName(student919));
					} catch (Exception e) {
						System.out.println(e);
					}
				} catch (Exception e) {
				...
				try {
					util.updateVehicleStatus(request.getParameter("vehicleId"),
							request.getParameter("status"));
					request.setAttribute("vehicles", util.getVehicles());
					forwardRequest("/manageVehicles.jsp", request, response);
				} catch (Exception e) {
				...
				try {
					...
					forwardRequest("/index.jsp?msg=Successfully Logged out!",
							request, response);
				} catch (Exception e) {
					System.out.println(e);
				}
				...
				try {
					...
					if (rideId != null && !rideId.equalsIgnoreCase("")) {
						...
						// for demonstration. remove thise lines after a midterm
						// presentation
						// if(status[0] == 0){
						// status[0] = 38;
						// }
						...
						forwardRequest("/rideStatus.jsp?rideId=" + rideId
								+ "&eta=" + eta + " MIN&stops="
								+ (int) status[1], request, response);
					} else {
						if (user.getUserType().equalsIgnoreCase("STUDENT")) {
							forwardRequest(
									"/studentHome.jsp?msg=You don't have an open ride request or you are already being served the request",
									request, response);
						}
				...
				try {
					...
					int noOfActiveVehicles = 1;
					int noOfReqs = 0;
					status = util.getRideStatusForDispatcher(
							request.getParameter("student919"),
							request.getParameter("phone"));
					if (status != null) {
						try {
							driverVehicleMap = (HashMap<String, String>) getServletContext()
						...
						forwardRequest("/rideStatus.jsp?rideId=" + eta
								+ "&stops=" + (int) noOfReqs, request, response);
					} else {
						forwardRequest(
								"/dispatcherCheckRideStatus.jsp?msg=No ride request exists with the given details",
								request, response);
					}
				} catch (Exception e) {
					System.out.println(e);
				...
				try {
					...
					if (resp.equalsIgnoreCase("MAILSENT")) {
						forwardRequest(
								"/index.jsp?msg=An email has been sent to you with the steps to reset your password.",
								request, response);
						...
						forwardRequest(
								"/forgotPassword.jsp?msg=Phone# and/or Email does not match with the username",
								request, response);
					}
				} catch (Exception e) {
					System.out.println(e);
				...
				try {
					...
					if (user != null) {
						...
						forwardRequest(
								"/index.jsp?msg=Oops! Something went wrong. Please try again later or contact Admin.",
								request, response);
					}
				} catch (Exception e) {
					System.out.println(e);
				...
				try {
					if (status != null && status.equalsIgnoreCase("true")) {
						getServletContext().setAttribute("serviceStatus",
								"true");
						session.setAttribute("serviceStatus", "true");
					} else {
				...
				try {
					if (adminMessage != null
							&& !"".equalsIgnoreCase(adminMessage.trim())) {
					...
					if (user.getUserType().equalsIgnoreCase("ADMIN")) {
						forwardRequest(
								"/adminHome.jsp?msg=Message successfully posted on the Home Page",
								request, response);
					} else if (user.getUserType()
							.equalsIgnoreCase("DISPATCHER")) {
						forwardRequest(
								"/dispatcherHome.jsp?msg=Message successfully posted on the Home Page",
								request, response);
					}
				} catch (Exception e) {
					System.out.println(e);
				...
				try {
					...
					if (user.getUserType().equalsIgnoreCase("ADMIN")) {
						forwardRequest(
								"/adminHome.jsp?msg=Message on the Home Page has been removed",
								request, response);
					} else if (user.getUserType()
							.equalsIgnoreCase("DISPATCHER")) {
						forwardRequest(
								"/dispatcherHome.jsp?msg=Message on the Home Page has been removed",
								request, response);
					}
				} catch (Exception e) {
					System.out.println(e);
				...
				try {
					...
					reportUtil = new SafeRideHomeReportService();
					...
					workbook = reportUtil.generateReport(fromDate, toDate,
							fromTime, toTime, interval);
					fullyQualifiedFileName = getServletContext().getRealPath(
							"/")
							+ "files/SafeRideHomeReport.xls";
					reportStream = new FileOutputStream(new File(
							fullyQualifiedFileName));
					// reportStream = new FileOutputStream(new
					// File("C:/1/a.xls"));
					workbook.write(reportStream);
					reportStream.close();
					forwardRequest("/generateReport.jsp?result=success",
							request, response);
				} catch (Exception e) {
					System.out.println(e);
					forwardRequest(
							"/generateReport.jsp?msg=An error occured while generating report",
							request, response);
				}
			}
		} catch (Exception e) {
	...
	public void forwardRequest(String url, HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException {
		try {
